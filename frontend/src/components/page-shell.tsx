...
function AnnouncementBanner({ html }: { html: string }) {
	...
	return (
		<div className="w-full bg-gradient-to-r from-sakura/20 via-lavender/20 to-sky/20 border-b border-sakura/30 px-4 py-2 flex items-center justify-between gap-3 text-sm">
			...
			<button
				...
				className="shrink-0 text-muted-foreground hover:text-foreground transition-colors text-xs px-1"
				aria-label="关闭公告"
			>
				✕
			</button>
		...
		<div className="min-h-dvh">
			...
			{config && config.jwt_secret_configured === false && (
				<div className="mx-auto max-w-5xl px-4 mt-3">
					<div className="rounded-xl border border-yellow-400/50 bg-yellow-50/80 dark:bg-yellow-900/20 px-4 py-3 text-sm text-yellow-800 dark:text-yellow-200 backdrop-blur-sm">
